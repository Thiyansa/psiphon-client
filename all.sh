...
# Supported regions and their descriptions
...
show_menu() {
    ...
    echo -e "   ${BLUE}7)${NC} Uninstall Psiphon"
...
uninstall_psiphon() {
    show_header
    echo -e "${RED}[!] WARNING: This will stop and remove all Psiphon setups.${NC}"
...
start_region() {
    ...
    if [ "$port" -eq 0 ]; then
        echo -e "${RED}[-231d] Invalid Region: $r${NC}"
        return
    fi

    local conf_file="$BASE_DIR/psiphon_$r.config"
    
    # Generate custom config for this region
    ...
    # Safely stop existing one if running
    sudo screen -S "psiphon_$r" -X quit >/dev/null 2>&1
    
    # Start under background screen
    sudo screen -dmS "psiphon_$r" "$BASE_DIR/psiphon" -config "$conf_file"
    
    echo -e "  - Region ${GREEN}$r${NC} started on Port: ${BOLD}${CYAN}$port${NC}"
    
    # --- SAFE VPS DELAY ---
    # එකපාර VPS එකට බරක් නොවී ආරක්ෂිතව run වීමට තත්පර 0.5 ක විරාමයක්.
    sleep 0.5 
}
...
start_custom_regions() {
    ...
    # Convert input to uppercase and replace spaces
...
psiphon_status() {
    ...
    for r in "${REGIONS[@]}"; do
        ...
        # Extract PID of the screen session if running
