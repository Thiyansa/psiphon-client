...
show_menu() {
    ...
    echo -e "   ${BLUE}3)${NC} Start ${GREEN}SPECIFIC / MULTIPLE${NC} Regions"
    echo -e "   ${BLUE}4)${NC} Stop ${RED}ALL${NC} Regions"
    echo -e "   ${BLUE}5)${NC} Stop ${RED}SPECIFIC${NC} Regions"
...
start_custom_regions() {
    ...
    # දැනට ක්‍රියාත්මක නොවන රටවල් සොයාගැනීම
    ...
    echo -e "------------------------------------------------------------------"
    echo -e "Tips: You can enter a single region (e.g. ${GREEN}US${NC}) or multiple regions separated by comma (e.g. ${GREEN}US,GB,JP${NC})"
    echo
    ...
    echo -e "\n${YELLOW}[*] Starting selected regions...${NC}"
...
stop_specific_region() {
    ...
    # දැනට දුවන සක්‍රිය රටවල් ටික සෙවීම
    ...
    echo -e "------------------------------------------------------------------"
    echo -e "Tips: You can enter a single region (e.g. ${RED}US${NC}) or multiple regions separated by comma (e.g. ${RED}US,GB,JP${NC})"
    echo
    ...
    IFS=',' read -ra selected_arr <<< "$input"
    echo -e "\n${YELLOW}[*] Stopping selected regions...${NC}"
    for r in "${selected_arr[@]}"; do
        if [[ " ${running_regions[*]} " =~ " ${r} " ]]; then
            sudo screen -S "psiphon_$r" -X quit >/dev/null 2>&1
            echo -e "  - Region ${RED}$r${NC} stopped successfully."
        else
...
psiphon_status() {
    ...
    for r in "${REGIONS[@]}"; do
        ...
        if [ -n "$pid" ]; then
            # Format and align with custom status string
